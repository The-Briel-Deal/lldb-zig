...
const clang::TargetInfo *TypeSystemZig::GetTargetInfo() {
  if (!m_target_info_up) {
    llvm::IntrusiveRefCntPtr<clang::DiagnosticIDs> diag_id_sp(
        new clang::DiagnosticIDs);
    m_diagnostic_options_up = std::make_unique<clang::DiagnosticOptions>();
    m_diagnostics_engine_up = std::make_unique<clang::DiagnosticsEngine>(
        diag_id_sp, *m_diagnostic_options_up);

    m_diagnostic_consumer_up = std::make_unique<NullDiagnosticConsumer>();
    m_diagnostics_engine_up->setClient(m_diagnostic_consumer_up.get(), false);

    m_target_options_sp = std::make_shared<clang::TargetOptions>();
    m_target_options_sp->Triple = m_arch.GetTriple().getTriple();
    m_target_info_up.reset(clang::TargetInfo::CreateTargetInfo(
        *m_diagnostics_engine_up, *m_target_options_sp));
  }
...
llvm::Expected<uint32_t>
TypeSystemZig::GetIndexOfChildWithName(opaque_compiler_type_t type,
                                       llvm::StringRef name,
                                       bool omit_empty_base_classes) {
  if (!type || name.empty())
...
size_t TypeSystemZig::GetIndexOfChildMemberWithName(
  ...
  if (auto idx_or_err =
    ...
    child_indices.push_back(*idx_or_err);
  else if (ZigPointerType *zig_pointer_type =
               llvm::dyn_cast_if_present<ZigPointerType>(UnwrapType(type)))
    switch (zig_pointer_type->GetSize()) {
    case ZigPointerType::Size::One:
      if (IsAggregateType(zig_pointer_type->GetChildType()->AsOpaqueType()))
        if (auto idx_or_err = GetIndexOfChildWithName(
